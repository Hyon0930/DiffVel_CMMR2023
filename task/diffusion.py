...
class SpecRollDiffusion(pl.LightningModule):
    ...
    def test_step_custom(self, batch, batch_idx):
...
#                       [127]*len(p_est))            
    ...
    def predict_step_custom(self, batch, batch_idx):
        noise = batch[0]
        waveform = batch[1]
        frame_roll = batch[2]
        # if self.hparams.inpainting_f or self.hparams.inpainting_t:
        ...
        for t_index in reversed(range(0, self.hparams.timesteps)):
            noise, spec = self.reverse_diffusion(noise, waveform, frame_roll, t_index)
            noise_npy = noise.detach().cpu().numpy()
                    # self.hparams.timesteps-i is used because slide bar won't show
                    # if global step starts from self.hparams.timesteps
            noise_list.append((noise_npy, t_index))                       
    ...
    def step(self, batch):
        ...
        if isinstance(batch, list):
            ...
            roll = self.normalize(batch[0]["velocity"]).unsqueeze(1) 
            frame_roll = self.normalize(batch[0]["frame"]).unsqueeze(1) 
            waveform = batch[0]["audio"]
            roll2 = self.normalize(batch[1]["velocity"]).unsqueeze(1) 
            frame_roll2 = self.normalize(batch[1]["frame"]).unsqueeze(1) 
            waveform2 = batch[1]["audio"]
            device = roll.device
        else:
            batch_size = batch["frame"].shape[0]
            roll = self.normalize(batch["velocity"]).unsqueeze(1) 
            frame_roll = self.normalize(batch["frame"]).unsqueeze(1) 
            waveform = batch["audio"]
            device = roll.device
        ...
        if self.hparams.training.mode == 'epsilon':
            if self.hparams.debug==True:
                epsilon_pred, spec = self(x_t, roll, frame_roll, t) # predict the noise N(0, 1)
            else:
                epsilon_pred, spec = self(x_t, waveform, frame_roll, t) # predict the noise N(0, 1)
            ...
            pred_roll, spec = self(x_t, waveform, frame_roll, t) # predict the noise N(0, 1)
            ...
            if isinstance(batch, list): # when using multiple dataset do one more feedforward
                ...
                x_t2 = q_sample( # sampling noise at time t
                    ...
                    sqrt_one_minus_alphas_cumprod=self.sqrt_one_minus_alphas_cumprod,
                    noise=noise)                
                pred_roll2, spec2 = self(x_t2, waveform2, frame_roll2, t, sampling=True) # sampling = True
    ...
    def sampling(self, batch, batch_idx):
        ...
        for t_index in reversed(range(0, self.hparams.timesteps)):
            if self.hparams.debug==True:
                noise, spec = self.reverse_diffusion(noise, roll, frame_roll,t_index)
            else:
                noise, spec = self.reverse_diffusion(noise, waveform, frame_roll, t_index)
            noise_npy = noise.detach().cpu().numpy()
                    # self.hparams.timesteps-i is used because slide bar won't show
    ...
    def ddpm(self, x, waveform, frame_roll, t_index):
        # x is Guassian noise
        ...
        # Equation 11 in the paper
        # Use our model (noise predictor) to predict the mean 
        epsilon, spec = self(x, waveform, frame_roll, t_tensor)
    ...
    def ddpm_x0(self, x, waveform, frame_roll, t_index):
        ...
        # Equation 11 in the paper
        # Use our model (noise predictor) to predict the mean 
        x0_pred, spec = self(x, waveform, frame_roll, t_tensor)
    ...
    def ddim_x0(self, x, waveform, frame_roll, t_index):
        # x is x_t, when t=T it is pure Gaussian
    ...
    def ddim(self, x, waveform, frame_roll, t_index):
        # boardcasting t_index into a tensor
        t_tensor = torch.tensor(t_index).repeat(x.shape[0]).to(x.device)
    ...
    def ddim2ddpm(self, x, waveform, frame_roll, t_index):
        # boardcasting t_index into a tensor
        t_tensor = torch.tensor(t_index).repeat(x.shape[0]).to(x.device)
...
#             return (model_mean + torch.sqrt(posterior_variance_t) * noise), spec     
    ...
    def cfdg_ddpm_x0(self, x, waveform, frame_roll, t_index):
        # x is x_t, when t=T it is pure Gaussian
        ...
        # Equation 11 in the paper
        # Use our model (noise predictor) to predict the mean 
        x0_pred_c, spec = self(x, waveform, frame_roll, t_tensor)
        x0_pred_0, _ = self(x, torch.zeros_like(waveform), frame_roll, t_tensor, sampling=True) # if sampling = True, the input condition will be overwritten
...
#         x0_pred = x0_pred_c
    ...
    def generation_ddpm_x0(self, x, waveform, frame_roll, t_index):
        # x is x_t, when t=T it is pure Gaussian
...
#         x0_pred = x0_pred_c
    ...
    def inpainting_ddpm_x0(self, x, waveform, frame_roll, t_index):
        # x is x_t, when t=T it is pure Gaussian
        ...
        # Equation 11 in the paper
        # Use our model (noise predictor) to predict the mean 
        x0_pred_c, spec = self(x, waveform, frame_roll, t_tensor, inpainting_t=self.hparams.inpainting_t, inpainting_f=self.hparams.inpainting_f)
        x0_pred_0, _ = self(x, torch.zeros_like(waveform), frame_roll, t_tensor, sampling=True) # if sampling = True, the input condition will be overwritten
        x0_pred = (1+self.hparams.sampling.w)*x0_pred_c - self.hparams.sampling.w*x0_pred_0
#         x0_pred = x0_pred_c
        # x0_pred = x0_pred_0
    ...
    def cfdg_ddim_x0(self, x, waveform, frame_roll, t_index):
        # x is x_t, when t=T it is pure Gaussian
        ...
        # Use our model (noise predictor) to predict the mean 
        x0_pred_c, spec = self(x, waveform, frame_roll, t_tensor)
        x0_pred_0, _ = self(x, torch.zeros_like(waveform), frame_roll, t_tensor)
        x0_pred = (1+self.hparams.sampling.w)*x0_pred_c - self.hparams.sampling.w*x0_pred_0
#         x0_pred = x0_pred_c
        # x0_pred = x0_pred_0
