...
class ResidualBlockwithFilm(nn.Module):
    ...
    def forward(self, x, diffusion_step, gamma, beta, conditioner=None):
        ...
        # Insert FiLM (score) conditions; beta and gammas for each residual layer. 
...
class ClassifierFreeDiffRoll(SpecRollDiffusion):
    def __init__(self,
        ...
        # For the double conditioned model, the condition below is 'fixed'.
        if condition == 'trainable_spec':
            trainable_parameters = torch.full((spec_args.n_mels,641), -1).float()
        ...
        if condition == 'trainable_z':
            ...
            #residual blocks to be conditioned on score information (MIDI frame roll)
            self.residual_layers = nn.ModuleList([
                ResidualBlockwithFilm(n_mels, residual_channels, dilation_base**(i % dilation_bound), kernel_size, uncond=unconditional)
                for i in range(residual_layers)
            ])
        ...
        self.film_layer = nn.Linear(in_features= 640*88, out_features= 2*residual_layers, bias= True)
    ...
    def forward(self, x_t, waveform, frame_roll, diffusion_step, sampling=False, inpainting_t=None, inpainting_f=None):
        #initial Gaussian noise input to the diffusion model
        x_t = x_t.squeeze(1).transpose(1,2)

        # Audio input/condition = waveform (B, L)
        # Score input/condition; MIDI frame roll (Batch, 1(=channel), Time(), Frames))
        frameroll_flat = torch.flatten(frame_roll, start_dim = 1)
        # frameroll_flat = frameroll_flat[:, :-88] #adjusting only for testing purposes due to dataloder difference; trimming 1 segment. 

        # Generate FiLM conditions (beta and gamma) by FiLM generator (Linear layer)
        film_feat = self.film_layer(frameroll_flat)
        ...
        # insert the conditions to residual layers; score and audio.
        for layer in self.residual_layers:
            index += 1

            gamma = film_feat[:, 2*(index-1)]
            beta = film_feat[:, 2*index-1]
            x, skip_connection = layer(x, diffusion_step, gamma, beta, spectrogram)
            skip = skip_connection if skip is None else skip_connection + skip
